...
function emptyQueueFilter (err, client, packet) {
  ...
  if (client.clean || !authorized) {
    ...
    write(client, packet, ()=>{
      next();
      client.broker.emit('afterForward', client, packet)
    })
  }
}

function finishConnect (arg, done) {
  done(); // no need to block broker flow
  this.client.broker.emit('finishConnect', this.client)
}
